/***********************************************************************
CustomMapGame.cpp - Controller for a Sandbox mapping / Island game
Copyright (c) 2017  Rasmus R. Paulsen (people.compute.dtu.dk/rapa)
adapted by William Kang
...
#include "CustomMapGame.h"

#include <string>
//#include <direct.h>

CCustomMapGame::CCustomMapGame()
{
	DataBaseDir = "customMapGame/";
	setDebug(true);
	...
	referenceMapHandler.Init("customMapGame/ReferenceData/");
	ShowScore = false;
	ButtonPressTime = -1;
	...
	{
		ofLogVerbose("CCustomMapGame()") << "could not read splash screen ";
	}
...
void CCustomMapGame::setup(std::shared_ptr<KinectProjector> const& k)
{
	kinectProjector = k;
...
void CCustomMapGame::PlayAndShowCountDown(int resultTime)
{
...
void CCustomMapGame::DrawScoresOnFBO()
{
	fboProjWindow.begin();
...
void CCustomMapGame::DrawScoreTexts()
{
...
void CCustomMapGame::drawProjectorWindow()
{
	if (ShowScore)
	{
...
bool  CCustomMapGame::InitiateGameSequence()
{
	eGameState sequence = GameSequence[CurrentGameSequence];
...
bool CCustomMapGame::CreateIntermediateResult()
{
...
bool CCustomMapGame::CheckForIsland()
{
...
bool CCustomMapGame::CreateFinalResult()
{
	if (!GetActualBinaryLandImage())
		return false;
...
bool CCustomMapGame::CreateWelcomeScreen()
{
	...
	if (DrawRefImage)
	{
		...
		{
			ofLogVerbose("CCustomMapGame") << "CreateWelcomeScreen(): could not read " + referenceImage;
			return false;
		}
		...
		if (sW == 0 || sH == 0)
		{
			ofLogVerbose("CCustomMapGame") << "splash image invalid";
			return false;
		}
...
bool CCustomMapGame::CreateSplashScreen()
{
	CheckedForIsland = 0;
...
bool CCustomMapGame::ButtonPressed()
{
	eGameState sequence = GameSequence[CurrentGameSequence];
...
bool CCustomMapGame::EndButtonPressed()
{
	CurrentGameSequence++;
	// Debug check
	if (CurrentGameSequence >= GameSequence.size())
...
void CCustomMapGame::RealTimeTestMe()
{
	if (!GetActualBinaryLandImage())
		return;
...
void CCustomMapGame::DebugTestMe()
{
	if (!ConnectedComponentAnalysis())
		return;
...
bool CCustomMapGame::isIdle()
{
	return(GameSequence[CurrentGameSequence] == GAME_STATE_IDLE);
}


void CCustomMapGame::setProjectorRes(ofVec2f& PR)
{
	projRes = PR;
...
void CCustomMapGame::setKinectRes(ofVec2f& KR)
{
	kinectRes = KR;
}

void CCustomMapGame::setKinectROI(ofRectangle& KROI)
{
	kinectROI = KROI;
	ROI.x = kinectROI.x;
	ROI.y = kinectROI.y;
...
ofRectangle CCustomMapGame::getKinectROI()
{
	return kinectROI;
}

void CCustomMapGame::setDebug(bool flag)
{
	debugOn = flag;

	if (flag)
	{
		std::string BaseDir = "data//customMapGame//DebugFiles//";
		// TODO: Find a way to create folder both on Win/Mac
		//		mkdir(BaseDir.c_str());

		// By default the application looks in data
		debugBaseDir = "customMapGame//DebugFiles//";
	}
}

bool CCustomMapGame::GetActualBinaryLandImage()
{
...
void CCustomMapGame::RemoveBorderBlobs(cv::Mat& image)
{
...
bool CCustomMapGame::WriteGreyOpenCVMat(cv::Mat& img, const std::string& fname)
{
	cv::Mat t;
...
bool CCustomMapGame::WriteRGBOpenCVMat(cv::Mat& img, const std::string& fname)
{
	cv::Mat t;
	img.copyTo(t);  // Do a temporary copy to avoid trouble with ROI definitions and data boundaries
...
void CCustomMapGame::SetupGameSequence()
{
...
bool CCustomMapGame::ComputeWhereInSequence(int& currentStep, int& totalSteps)
{
	totalSteps = 0;
	currentStep = 0;
...
bool CCustomMapGame::ConnectedComponentAnalysis()
{
	...
	if (BinaryLandImage.width == 0)
	{
		...
		{
			ofLogVerbose("CCustomMapGame") << "ConnectedComponentAnalysis(): could not read BinaryLandImage.png";
			return false;
		}
	}
	else
	...
	if (contours.size() == 0)
	{
		ofLogVerbose("CCustomMapGame") << "ConnectedComponentAnalysis(): could not find any contours";
		return false;
	}
	int MaxCont = 0;
	int maxID = -1;
	...
	ofLogVerbose("CCustomMapGame") << "Found max contour with " + ofToString(MaxCont) + " points";
	...
	{
		ofLogVerbose("CCustomMapGame") << "ConnectedComponentAnalysis(): Max BLOB area too small";
		return false;
	}
...
void CCustomMapGame::ComputeLandmarksFromTemplate(cv::Mat& I, std::vector<cv::Point2f>& LMs)
{
	LMs.clear();
...
bool  CCustomMapGame::CheckCollinear(cv::Point2f p1, cv::Point2f p2, cv::Point2f p3)
{
	cv::Point2f a = p2 - p1;
	cv::Point2f b = p3 - p1;
...
bool CCustomMapGame::SelectFourGoodLms(std::vector<cv::Point2f>& LMs, std::vector<int>& goodIds)
{
	int xmin = ROI.x;
	int ymin = ROI.y;
...
bool CCustomMapGame::ComputeCM(const cv::Mat& I, double& CMx, double& CMy)
{
	CMx = 0;
	CMy = 0;
	int N = 0;
...
bool CCustomMapGame::ReferenceToCM(cv::Mat& I, cv::Mat& Iout, double& rCMx, double& rCMy)
{
	rCMx = 0;
	rCMy = 0;
...
bool CCustomMapGame::SafeReadBinaryImage(cv::Mat& img, const std::string& fname)
{
	ofImage temp;
	if (!temp.loadImage(fname))
	{
		ofLogVerbose("CCustomMapGame") << "SafeReadBinaryImage(): could not read " + fname;
		return false;
	}
...
void CCustomMapGame::CompareImages(cv::Mat& I, cv::Mat& T, int Xi, int Yi, cv::Mat& MatchImage, double& DSC)
{
	int dX = (Xi - T.cols / 2);
	int dY = (Yi - T.rows / 2);
...
bool CCustomMapGame::ReadReferenceLMRaw(std::vector<cv::Point2f>& LMs, std::string& referenceLM)
{
...
bool CCustomMapGame::EstimateBorderColor(cv::Mat& RefImage, cv::Scalar& borderColor)
{
	double Rsum = 0;
	double Gsum = 0;
	double Bsum = 0;
...
std::string CCustomMapGame::GetDateTimeString()
{
...
bool CCustomMapGame::UpdateScores()
{
...
bool CCustomMapGame::CreateScoreTexts(bool intermediate)
{
	int actRef = referenceMapHandler.GetActualRef();
	//std::string RefMap = referenceMapHandler.ReferenceMaps[actRef];
	std::string RefMapName = referenceMapHandler.ReferenceNames[actRef];
...
bool CCustomMapGame::ComputeProjectorMap()
{
	...
	if (!temp.loadImage(referenceImage))
	{
		ofLogVerbose("CCustomMapGame") << "ComputeProjectorMap(): could not read " + referenceImage;
		return false;
	}
...
void CCustomMapGame::DrawMatchResultContourLines()
{
	ofSetColor(255, 0, 0);
